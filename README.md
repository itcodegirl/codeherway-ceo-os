...
- **Cross-tab list refresh** — `useCrudPage` subscribes to its repository's `*_UPDATED_EVENT` so promotions from Capture (or any other tab) silently re-fetch an open Opportunities or Content OS list without a manual reload.
...
Coverage for the above ships in this branch: 398 unit/integration tests, lint, typecheck, build, and route-budget checks all pass. The `@axe-core/playwright` sweep ships as a Playwright spec — exercise it on a machine with browsers installed (`npx playwright install` first).
