...
- **Cross-page promotion** — each Capture sticky has a "Make reminder" action and each pending Focus Home reminder has a "Promote" action that creates a weekly priority. Both use the existing repositories, fire calm toast confirmations, and leave the source record in place so the user decides whether to keep the long-form context.
...
Coverage for the above ships in this branch: 380 unit/integration tests, lint, typecheck, build, and route-budget checks all pass. The `@axe-core/playwright` sweep ships as a Playwright spec — exercise it on a machine with browsers installed (`npx playwright install` first).
