...
- **Server-side optimistic locking coverage.** Supabase-backed Opportunity and Content OS updates use `updated_at` equality checks, and local-first stale-write detection exists across CRUD/Weekly surfaces. Weekly Brief Supabase items, Settings, and Chief workspace still need the same authenticated conflict contract validated against staging.
