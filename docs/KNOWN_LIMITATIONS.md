...
- **Authenticated conflict coverage beyond Opportunity and Content OS.** Supabase-backed Opportunity and Content OS rows now carry `updated_at` through list/create/update paths and use equality checks on update. Weekly Brief Supabase items, Settings, and Chief workspace still need the same authenticated conflict contract validated against staging.
